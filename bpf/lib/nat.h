							 ...
							 __u32 off)
{
	...
	/* This erroff is L4 offset of inner packet, where off is L4
	 * offset of outer packet.
	 *  off                     erroff
	 *   [ ICMP ERROR | INNER IP | INNER L4 ...]
	 */
	...
	if (state->to_dport != tuple->dport) {
		...
		case IPPROTO_ICMP: {
			...
			if (ctx_store_bytes(ctx, erroff +
					    ...
					    sizeof(state->to_dport), 0) < 0)
				return DROP_WRITE_ERROR;
			/* TODO(sahid): There is design issue with
			 * csum_l4_offset_and_flags as we have to force setting
			 * attributes for ICMP. We should consider review this
			 * part.
			 */
		...
		if (csum_l4_replace(ctx, erroff, &csum, 0, suml4, 0) < 0)
			return DROP_CSUM_L4;
	}
	...
	if (ctx_store_bytes(ctx, off + sizeof(struct icmphdr) + offsetof(struct iphdr, saddr),
		...
		return DROP_WRITE_ERROR;
	if (l3_csum_replace(ctx, off + sizeof(struct icmphdr) + offsetof(struct iphdr, check),
			    0, sum, 0) < 0)
		return DROP_CSUM_L3;
	return 0;
}
...
snat_v4_rev_nat(struct __ctx_buff *ctx, const struct ipv4_nat_target *target)
{
	...
	int ret;
	__u8 nexthdr;
	bool icmp_rewrite_embedded = false;
	...
	case IPPROTO_ICMP:
		...
		case ICMP_DEST_UNREACH:
			...
			case ICMP_FRAG_NEEDED: {
				 ...
				 *
				 * TODO(sahid): We could certainly support more ICMP
				 * Error using this function.
				 */
				icmp_rewrite_embedded = true;
				break;
			}
			default:
	...
	return snat_v4_rewrite_ingress(ctx, &tuple, state, off, nexthdr);
}
#else
			   ...
			   const struct ipv6_nat_target *target)
{
	...
	*state = snat_v6_lookup(tuple);
	ret = snat_v6_track_local(ctx, tuple, *state, NAT_DIR_EGRESS, off, target);
	if (ret < 0)
		return ret;
	else if (*state)
		return NAT_CONTINUE_XLATE;
			       ...
			       const struct ipv6_nat_target *target)
{
	...
	*state = snat_v6_lookup(tuple);
	ret = snat_v6_track_local(ctx, tuple, *state, NAT_DIR_INGRESS, off, target);
	if (ret < 0)
		return ret;
	else if (*state)
		return NAT_CONTINUE_XLATE;
							 ...
							 __u32 off)
{
	 ...
	 * offset of outer packet.
	 *
	 * NOTE(sahid): -4 because struct icmp6hdr includes echo
	 * identifier which is not used for ICMP Error.
	 *
	 * off                     erroff
	 *  [ ICMP ERROR | INNER IP | INNER L4 ...]
	 */
	erroff = off + sizeof(struct icmp6hdr) - 4 + hdrlen;
	...
	if (state->to_dport != tuple->dport) {
		 ...
		 */
		suml4 = csum_diff(&tuple->dport, 4, &state->to_dport, 4, 0);
		if (csum_l4_replace(ctx, erroff, &csum, 0, suml4, BPF_F_PSEUDO_HDR) < 0)
			return DROP_CSUM_L4;
	}
	/* Change IP of source address of inner packet to refer the
	 * endpoint.
	 */
	if (ctx_store_bytes(ctx, off + sizeof(struct icmp6hdr) - 4 +
...
snat_v6_rev_nat(struct __ctx_buff *ctx, const struct ipv6_nat_target *target)
{
	...
	case IPPROTO_ICMPV6:
		...
		case ICMPV6_PKT_TOOBIG: {
			 ...
			 */
			icmpoff += sizeof(icmp6hdr) - sizeof(icmp6hdr.icmp6_dataun.u_echo);
			...
			icmpoff += ipv6_hdrlen(ctx, &iphdr.nexthdr);
			switch (tuple.nexthdr) {
			case IPPROTO_TCP:
			case IPPROTO_UDP:
				if (ctx_load_bytes(ctx, icmpoff, &l4hdr, sizeof(l4hdr)) < 0)
					return DROP_INVALID;
				tuple.sport = l4hdr.dport;
				tuple.dport = l4hdr.sport;
				break;
	...
	if (icmp_rewrite_embedded) {
		/* Rewrite source of embedded packet returned by ICMP Error. */
		ret = snat_v6_icmp_rewrite_embedded(ctx, &tuple, state, off);
		if (ret < 0)
			return ret;
	}

	return snat_v6_rewrite_ingress(ctx, &tuple, state, off, nexthdr);
}
#else
static __always_inline __maybe_unused
